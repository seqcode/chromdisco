...
def _iterative_extract_chromlets(score_track, window_size, flank, suppress):
	...
	for example_idx, single_score_track in enumerate(score_track):
		while True:
			...
			if argmax >= flank and argmax < (d-flank): 
				seqlet = core.Chromlet(example_idx=example_idx, 
					start=argmax-flank, end=argmax+window_size+flank,
					is_revcomp=False)

				chromlets.append(seqlet)
...
def extract_chromlets(attribution_scores, window_size, flank, suppress, 
	...
	chromlets = _iterative_extract_chromlets(score_track=smoothed_tracks,
